...
def masses(true, assumed, function, a1, a2, weight):
  assumed_masses = []
  true_masses = []
  for i in xrange(int(round(5000*weight))):
    D = TLorentzVector()
...
def style_plot(plot, colour, ytitle="Candidates", xtitle="D^{0} Mass / MeV"):
...
ts_assumed = [ THStack("ts_assumed_"+rec_name,"ts_assumed_"+rec_name) for rec_name,hyp in mass_hytpotheses ]
ts_true = [ THStack("ts_true_"+rec_name,"ts_true_"+rec_name) for rec_name,hyp in mass_hytpotheses ]
...
#for type_name, function, colour_add in [("back",background,bargs,1),("sig",signal,[1.,1.],0)]:
...
n_tot = 0
n_acc = 0
for p_arr,true_mass in masses(true_hyp, [pi,pi], emu_sig, 1.,1.,1.):
  m = get_mass(p_arr)
  n_tot += 1
  #print m, true_mass
  #if True:
...
for i,(rec_name,hyp) in enumerate(mass_hytpotheses):
  ...
  style_plot(assumed_plot,colour+colour_add)
  ...
  if i==0:
    style_plot(true_plot,colour+colour_add)
    ...
    tc.SaveAs("true_mass_%s_%s_%s.pdf"%(gen_name,type_name,rec_name))
    tc.SaveAs("true_mass_%s_%s_%s.png"%(gen_name,type_name,rec_name))
